#!/usr/bin/env python3
"""Track 3 (Value Recovery) walk-forward backtest — RUSSELL1000, 1-month holding.

Usage
-----
    python scripts/run_backtest_track3.py
...
-------------------
  ...
  attach_p_fcf_history() requires the full snapshot dict to be built before the
  walk-forward loop begins.  It looks back up to 60 months per snapshot date.
  With a 24-month warm-up and min_history=12, p_fcf_vs_history values are available
  from the 13th snapshot onward (2014-01-31), well before the first test month.

Outputs
-------
  data/results/track3_RUSSELL1000_1m_report.md
  data/results/track3_RUSSELL1000_1m_contributions.md
  data/results/track3_RUSSELL1000_1m_picks.csv
"""
...
from crucible.snapshot import attach_momentum, attach_p_fcf_history, build_snapshots_parallel
...
def _run_track3_backtest(
    ...
    """Walk-forward backtest using Track 3 filters + scorer."""
...
def _generate_report(
    ...
    """Write Track 3 Markdown backtest report."""
    bt = result.bt_config
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Track 3 walk-forward backtest")
    parser.add_argument(
    ...
    try:
        ...
        # ── Walk-forward ──────────────────────────────────────────────────────
        log.info("Running Track 3 walk-forward backtest …")
        ...
        if not result.monthly_results:
            log.error(
                ...
                "Run diagnose_funnel.py --universe RUSSELL1000 --track 3 for details."
            )
            sys.exit(1)
        ...
        _generate_report(
            result,
            RESULTS_DIR / "track3_RUSSELL1000_1m_report.md",
            config,
        )

        generate_ticker_contribution(
            result,
            RESULTS_DIR / "track3_RUSSELL1000_1m_contributions.md",
            roic_threshold=config.track3_filters.roic_proxy_min,
        )

        generate_picks_csv(
            result, prices,
            RESULTS_DIR / "track3_RUSSELL1000_1m_picks.csv",
        )
        ...
        print("\n" + "═" * 65)
        print("  Track 3 Backtest — RUSSELL1000 — 1-month holding")
        print("═" * 65)
        ...
        print("═" * 65)
        print(f"\n  Report:        {RESULTS_DIR}/track3_RUSSELL1000_1m_report.md")
        print(f"  Contributions: {RESULTS_DIR}/track3_RUSSELL1000_1m_contributions.md")
        print(f"  Picks CSV:     {RESULTS_DIR}/track3_RUSSELL1000_1m_picks.csv")
        print()
